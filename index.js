const outer = function() {
  ...
  Promise.all([
    ...
    let subject, text;
    const [lowerResults, upperResults, northResults, testResults] = results;
    if (results.every((result) => result.countAvailableSites === 0)) {
      // no sites availabel anywhere
      subject = 'No sites available :('
      text = `No sites available at ${lowerResults.name}, ${upperResults.name}, ${northResults.name}${testResults ? ', ' + testResults.name : null}.`
    } else {
      // SITES AVAILABLE
...
outer();
setInterval(() => outer(), 4 * oneHourInMs);